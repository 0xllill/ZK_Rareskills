...
void Max3_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Max3_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 785;}
...
ctx->componentMemory[coffset].templateName = "Max3";
ctx->componentMemory[coffset].signalStart = soffset;
...
// load src
Fr_sub(&expaux[1],&circuitConstants[2],&signalValues[mySignalStart + 4]); // line circom 32
Fr_mul(&expaux[0],&expaux[1],&signalValues[mySignalStart + 5]); // line circom 32
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_sub(&expaux[1],&circuitConstants[2],&signalValues[mySignalStart + 5]); // line circom 36
Fr_mul(&expaux[0],&signalValues[mySignalStart + 6],&expaux[1]); // line circom 36
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_sub(&expaux[1],&circuitConstants[2],&signalValues[mySignalStart + 4]); // line circom 40
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 7],&signalValues[mySignalStart + 2]); // line circom 47
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 8],&signalValues[mySignalStart + 3]); // line circom 48
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 9],&signalValues[mySignalStart + 1]); // line circom 49
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
PFrElement aux_dest = &signalValues[mySignalStart + 0];
// load src
Fr_add(&expaux[1],&signalValues[mySignalStart + 10],&signalValues[mySignalStart + 11]); // line circom 51
Fr_add(&expaux[0],&expaux[1],&signalValues[mySignalStart + 12]); // line circom 51
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
for (uint i = 0; i < 3; i++){
